...
> Generate AI videos from text prompts via the HeyGen API. Text-to-video, image-to-video, and provider routing across VEO, Kling, Sora, Runway, and Seedance.
...
Use when generating video from a text description, image-to-video with a reference frame, or picking the right provider per use case.

## Install

```bash
# Claude Code plugin install (one-line)
claude plugin install ai-video-gen --from https://fadaly.net/downloads/skills/ai-video-gen.zip
```

Or clone this repo into your local Claude skills directory (typically `~/.claude/skills/` on macOS / Linux):

```bash
git clone https://github.com/MomoFadaly/ai-video-gen.git
```

Or download the zip from [fadaly.net/skills/ai-video-gen](https://fadaly.net/skills/ai-video-gen) (the per-skill landing page on fadaly.net) and extract into `~/.claude/skills/`.
...
- VEO · Kling · Sora · Runway · Seedance — underlying video providers
